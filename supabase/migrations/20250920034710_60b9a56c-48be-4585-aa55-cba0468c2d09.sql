...
-- 1. Add your admin account to admin_roles table (handle duplicate properly)
INSERT INTO public.admin_roles (user_email, role, is_active) 
VALUES ('saefalib@innosinlab', 'admin', true)
ON CONFLICT DO NOTHING;

-- If exists, update to ensure active
UPDATE public.admin_roles 
SET is_active = true 
WHERE user_email = 'saefalib@innosinlab' AND role = 'admin';
...
BEGIN
  ...
  -- Allow max 5 anonymous sessions per hour globally
  RETURN session_count < 5;
END;
$$ LANGUAGE PLPGSQL SECURITY DEFINER SET search_path = public;

-- 7. Enhanced RLS policy for admin access (drop conflicting first)
