...
const resolveCollisions = (newProduct: PlacedProduct, existingProducts: PlacedProduct[]) => {
  const buffer = 0.01; // minimum separation distance
  let moved = false;

  for (const product of existingProducts) {
    const dx = newProduct.position.x - product.position.x;
    const dz = newProduct.position.y - product.position.y;
    const distance = Math.sqrt(dx * dx + dz * dz);
    if (distance < buffer) {
      newProduct.position.x += buffer;
      newProduct.position.y += buffer;
      moved = true;
    }
  }
  return moved;
...
const EnhancedCanvasWorkspace3D: React.FC<EnhancedCanvasWorkspace3DProps> = ({
  ...
  const handleWallClick = useCallback((wallId: string) => {
    if (currentMode === 'select') {
      ...
      if (wall && onWallUpdate) {
        const newLength = prompt("Enter new wall length in meters:", "");
        if (newLength) {
          ...
          const newEnd = {
            x: start.x + unit.x * parseFloat(newLength),
            y: start.y + unit.y * parseFloat(newLength),
          };
  ...
  const handleCanvasDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      const snappedX = snapToGrid(point.x, 1);
      const snappedY = snapToGrid(point.z, 1);
      ...
      resolveCollisions(newProduct, placedProducts);
  ...
  }, [setPlacedProducts, placedProducts]);
